...
if __name__ == '__main__':
    ...
    for i, dir in enumerate(["cw"]):
        path_human = HumanPath(0, 0, math.pi / 4 * 5)
        path_human.appendStraight(-b, -b)
        path_human.appendCircular(r, -math.pi/2 * 3)
        path_human.appendStraight(b * math.sqrt(2), -b * math.sqrt(2))
        path_human.appendCircular(r, math.pi/2 * 3)
    ...
    if args.preview:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(os.getenv("HOME"), "Pictures", "figs", "human_robot_path8.eps"))
        plt.show()