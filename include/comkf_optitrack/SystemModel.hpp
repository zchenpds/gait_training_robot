...
public:
    ...
    //! X-CoP
    static constexpr size_t COPX = 6;
    //! Y-CoP
    static constexpr size_t COPY = 7;
...
protected:
    ...
    {        
        ...
        this->F.template block<2, 2>(S::VX, S::COPX) = Kalman::Matrix<T, 2, 2>::Identity() * (-omega0sq_ * dt_);

        this->F.template block<2, 2>(S::BX, S::BX).setIdentity();

        this->F.template block<2, 2>(S::COPX, S::COPX).setIdentity();
